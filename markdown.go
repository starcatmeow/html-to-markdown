...
type Options struct {
	...
	// The start symbols of escaped block. Note that program will match these symbols one by one
	// until matched, then use the same index for end symbol.
	// Set to [] to disable escaped block feature.
	// default: []
	EscapeStart []string

	// The end symbols of escaped block. Set to [] to disable escaped block feature.
	// default: []
	EscapeEnd []string
